...
const styles = StyleSheet.create({
  ...
  /* Legal disclaimer box (navy/slate — non-alarming) */
  legalDisclaimer: {
    backgroundColor: "#1E293B",
    borderWidth: 0.5,
    borderColor: "#334155",
  ...
  footerBrand: { fontSize: 10, fontFamily: "Helvetica-Bold", color: C.navy },
  footerBrandAccent: { color: C.yellow },
  footerSub: { fontSize: 8, color: C.gray },
  footerRight: { flexDirection: "column", alignItems: "flex-end", gap: 2 },
  footerMeta: { fontSize: 8, color: C.gray },
});
...
export interface WhiteLabelOptions {
  companyName: string;
  phone?: string;
  /** Base64 data URI for company logo (optional) */
  logoBase64?: string;
}
...
interface DocProps {
  ...
  diagramPng?: string;
  /** When provided, replaces VoltSpec branding with contractor info */
  whiteLabel?: WhiteLabelOptions;
}

/* ── Reusable components ────────────────────────────────────────────── */

function PageFooter({ dateStr, whiteLabel }: { dateStr: string; whiteLabel?: WhiteLabelOptions }) {
  return (
    <View style={styles.footer} fixed>
      ...
      <View style={styles.footerBody}>
        <View style={styles.footerLeft}>
          {whiteLabel ? (
            <>
              <Text style={styles.footerBrand}>{clean(whiteLabel.companyName)}</Text>
              {whiteLabel.phone && <Text style={styles.footerSub}>{whiteLabel.phone}</Text>}
            </>
          ) : (
            <>
              <Text style={styles.footerBrand}>
                <Text style={styles.footerBrandAccent}>⚡</Text> Generated by VoltSpec
              </Text>
              <Text style={styles.footerSub}>Generated by VoltSpec - Independent Electrical Estimating Tool | voltspec.online</Text>
            </>
          )}
        </View>
        <View style={styles.footerRight}>
          <Text style={styles.footerMeta}>{dateStr}</Text>
          <Text
...
function RunningHeader({ jobLabel, cityLabel, whiteLabel }: { jobLabel: string; cityLabel: string; whiteLabel?: WhiteLabelOptions }) {
  return (
    <View style={styles.runningHeader}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
        {whiteLabel?.logoBase64 ? (
          <Image src={whiteLabel.logoBase64} style={{ width: 16, height: 16 }} />
        ) : !whiteLabel ? (
          <Image src={VOLTSPEC_LOGO_BASE64} style={{ width: 16, height: 16 }} />
        ) : null}
        <Text style={styles.runningBrand}>
          {whiteLabel ? clean(whiteLabel.companyName) : <>Volt<Text style={styles.runningAccent}>Spec</Text></>}
        </Text>
      </View>
...
function LegalDisclaimerBlock({ compact = false }: { compact?: boolean }) {
  const boxStyle = compact
    ? { ...styles.legalDisclaimer, paddingHorizontal: 8, paddingVertical: 6, marginTop: 8 }
    : styles.legalDisclaimer;
  const textStyle = compact
    ? { ...styles.legalDisclaimerText, fontSize: 6.5 }
    : styles.legalDisclaimerText;
  const lastStyle = compact
    ? { ...styles.legalDisclaimerTextLast, fontSize: 6.5 }
    : styles.legalDisclaimerTextLast;
  const titleStyle = compact
    ? { ...styles.legalDisclaimerTitle, fontSize: 7 }
    : styles.legalDisclaimerTitle;

  return (
    <View style={boxStyle} wrap={false}>
      <Text style={titleStyle}>LEGAL NOTICE</Text>
      <Text style={textStyle}>
        VoltSpec is an independent tool developed by VoltSpec LLC and is not affiliated with, endorsed by, or sponsored by Elliott Electric Supply or any utility company.
      </Text>
      <Text style={textStyle}>
        All material lists, pricing, and availability shown are estimates only. Always verify current pricing, stock, and specifications with your local Elliott branch before ordering.
      </Text>
      <Text style={lastStyle}>
...
function VoltSpecDocument({ result, diagramPng, whiteLabel }: DocProps) {
    ...
    >
      ...
      <Page size="LETTER" style={styles.coverPage}>
        <View style={styles.coverStripe}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            {whiteLabel?.logoBase64 ? (
              <Image src={whiteLabel.logoBase64} style={{ width: 28, height: 28 }} />
            ) : !whiteLabel ? (
              <Image src={VOLTSPEC_LOGO_BASE64} style={{ width: 28, height: 28 }} />
            ) : null}
            <Text style={styles.coverBrand}>
              {whiteLabel ? clean(whiteLabel.companyName) : <>Volt<Text style={styles.coverBrandAccent}>Spec</Text></>}
            </Text>
          </View>
          <Text style={styles.coverTagline}>
            NEC 2026 Electrical Job Package - {clean(cityLabel)}
          </Text>
        </View>
        ...
        <PageFooter dateStr={fullDate} whiteLabel={whiteLabel} />
      </Page>
      ...
      <Page size="LETTER" style={styles.page} wrap>
        ...
        <View style={styles.pageBody}>
          <RunningHeader jobLabel={job.label} cityLabel={cityLabel} whiteLabel={whiteLabel} />
...
const ts = StyleSheet.create({
  ...
  /* ── Job Sheet footer — matches full PDF style ──────────────────── */
  footer: {
    position: "absolute",
    bottom: 0,
    left: 0,
  ...
  footerRight: { flexDirection: "column", alignItems: "flex-end", gap: 2 },
  footerMeta: { fontSize: 8, color: C.gray },
  footerDisclaimer: { fontSize: 8, fontFamily: "Helvetica-Bold", color: C.red },
});

function TruckSheetDocument({ result, whiteLabel }: { result: GenerateResult; whiteLabel?: WhiteLabelOptions }) {
    ...
    >
      <Page size="LETTER" style={ts.page}>
        ...
        <View style={ts.header}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            {whiteLabel?.logoBase64 ? (
              <Image src={whiteLabel.logoBase64} style={{ width: 20, height: 20 }} />
            ) : !whiteLabel ? (
              <Image src={VOLTSPEC_LOGO_BASE64} style={{ width: 20, height: 20 }} />
            ) : null}
            <Text style={ts.brand}>
              {whiteLabel ? (
                <>{clean(whiteLabel.companyName)}<Text style={{ fontSize: 8, color: C.gray }}> Job Sheet</Text></>
              ) : (
                <>Volt<Text style={ts.brandAccent}>Spec</Text><Text style={{ fontSize: 8, color: C.gray }}> Job Sheet</Text></>
              )}
            </Text>
          </View>
        ...
        {/* Legal Disclaimers */}
        <LegalDisclaimerBlock compact />

        {/* Footer — consistent with full PDF */}
        <View style={ts.footer} fixed>
          <View style={ts.footerBar} />
          <View style={ts.footerBody}>
            <View style={ts.footerLeft}>
              {whiteLabel ? (
                <>
                  <Text style={ts.footerBrand}>{clean(whiteLabel.companyName)}</Text>
                  {whiteLabel.phone && <Text style={ts.footerSub}>{whiteLabel.phone}</Text>}
                </>
              ) : (
                <>
                  <Text style={ts.footerBrand}>
                    <Text style={ts.footerBrandAccent}>⚡</Text> Generated by VoltSpec
                  </Text>
                  <Text style={ts.footerSub}>Generated by VoltSpec - Independent Electrical Estimating Tool | voltspec.online</Text>
                </>
              )}
            </View>
            <View style={ts.footerRight}>
              <Text style={ts.footerDisclaimer}>Verify with AHJ - Not engineering advice</Text>
              <Text style={ts.footerMeta}>{clean(job.label)} | {dateStr}</Text>
            </View>
          </View>
        </View>
      </Page>
...
export async function generateJobSheet(result: GenerateResult, whiteLabel?: WhiteLabelOptions): Promise<void> {
  const blob = await pdf(<TruckSheetDocument result={result} whiteLabel={whiteLabel} />).toBlob();
  const url = URL.createObjectURL(blob);
...
export async function generatePDF(result: GenerateResult, whiteLabel?: WhiteLabelOptions): Promise<void> {
  ...
  const blob = await pdf(
    <VoltSpecDocument result={result} diagramPng={diagramPng} whiteLabel={whiteLabel} />,
  ).toBlob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
